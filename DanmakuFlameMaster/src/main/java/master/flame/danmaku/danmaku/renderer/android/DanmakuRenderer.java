...
public class DanmakuRenderer extends Renderer {
    ...
    public void draw(final IDisplayer disp, IDanmakus danmakus, long startRenderTime, final RenderingState renderingState) {
        ...
        if (elapsed > 8 && renderingState.totalDanmakuCount > 10) {
            Log.w("DanmakuRenderer", String.format("draw() took %dms for %d danmakus (cache hits: %d, misses: %d)",
                elapsed, renderingState.totalDanmakuCount, renderingState.cacheHitCount, renderingState.cacheMissCount));
        }
    }
