...
public class CachingPolicy {
    ...
    // 现代设备 (API 26+) 优化：提高复用容差降低 GC 频率，缓存池比例保持保守
    public final static CachingPolicy POLICY_LAZY = new CachingPolicy(BMP_BPP_ARGB_8888, 0.25f, CACHE_PERIOD_AUTO, 5, 0.05f);
    public final static CachingPolicy POLICY_GREEDY = new CachingPolicy(BMP_BPP_ARGB_8888, 0.4f, CACHE_PERIOD_NOT_RECYCLE, 5, 0.02f);
...
//    public DanmakuTimer recyleTimer = new DanmakuTimer(SystemClock.uptimeMillis());
    ...
    /**
     * 可复用缓存的尺寸容差（像素），现代设备上调高可减少 Bitmap 分配次数
     * @see master.flame.danmaku.controller.CacheManagingDrawTask.CacheManager#findReusableCache
     */
    public int reusableOffsetPixel = 5;

    public int maxTimesOfStrictReusableFinds = 10;

    public int maxTimesOfReusableFinds = 100;
