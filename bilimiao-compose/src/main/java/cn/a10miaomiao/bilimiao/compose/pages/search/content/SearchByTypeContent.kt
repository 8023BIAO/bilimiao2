...
private class SearchByTypeContentViewModel(
    ...
    val categorySortList = listOf(
        SearchByTypeRequest.CategorySort.DEFAULT to "默认排序",
        SearchByTypeRequest.CategorySort.PUBLISH_TIME to "最新发布",
        SearchByTypeRequest.CategorySort.CLICK_COUNT to "最多点击",
        SearchByTypeRequest.CategorySort.LIKE_COUNT to "最多点赞",
        SearchByTypeRequest.CategorySort.COMMENT_COUNT to "最多评论",
    )
    val categorySort = mutableStateOf(categorySortList[0])
    ...
    private fun loadData(
        ...
        try {
            ...
            val req = SearchByTypeRequest(
                ...
                ) else if (type == 6) it.copy(
                    categorySort = categorySort.value.first,
                ) else it
            }
    ...
    fun menuItemClick(view: View, item: MenuItemPropInfo) {
        ...
        when (key) {
            ...
            in 30..39 -> {
                val sort = key - 30
                categorySort.value = categorySortList.find {
                    it.first.value == sort
                } ?: categorySortList[0]
                refresh()
            }
    ...
    fun toDetailPage(item: SearchItem) {
        val articleCard = item.article
        if (articleCard != null) {
            pageNavigation.navigate(ArticleReaderPage(articleCard.id))
            return
        }
...
private fun SearchByTypeContentConfig(
    ...
    val categorySort by viewModel.categorySort
    val pageConfigId = PageConfig(
        title = "搜索\n-\n$keyword",
        menu = rememberMyMenu(type, userSort, userType, categorySort) {
            myItem {
                key = MenuKeys.search
            ...
            if (type == 6) {
                myItem {
                    key = MenuKeys.sort
                    title = categorySort.second
                    iconFileName = "ic_baseline_filter_list_grey_24"
                    childMenu = myMenu {
                        checkable = true
                        checkedKey = 30 + categorySort.first.value
                        viewModel.categorySortList.forEach {
                            myItem {
                                title = it.second
                                key = 30 + it.first.value
                            }
                        }
                    }
                }
