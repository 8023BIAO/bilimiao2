 ...
 * |---|---|---|---|
 * | 双击暂停 | `live_double_tap_pause`（默认 true） | 用户："我都想默认就是开启双击暂停的"——默认行为不需要一个开关 | 默认就是开。★本轮更正：播放页弹窗起只放弹幕 4 项，所以这一项**当前没有 UI 入口**（值仍被播放页读取，见 `LivePlayerActivity.onDoubleTapPauseEnabled`） |
 ...
 *
 * ★★本轮（2026-09-26 晚，用户拍板"弹窗瘦身"）：**本页仍然是全部 9 项**
 ...
 *   一个都没少 —— 用户原话："它们仍在「设置 → 直播设置」页，设置页一个项都不能少。"
 *
