...
private class UserMedialistDetailViewMode(
    ...
    val searchKeyword = MutableStateFlow(keyword)
    val downloadDialogState = VideoDownloadDialogState(viewModelScope)
    private val _allData = mutableListOf<MediaListV2Info>()
    ...
    private fun loadData(
        ...
        try {
            ...
            if (res.isSuccess) {
                ...
                val mediaList = data.media_list
                if (mediaList != null) {
                    if (oid.isBlank()) {
                        _allData.clear()
                        _allData.addAll(mediaList)
                        applySearchFilter()
                    } else {
                        val newItems = mediaList.filter { i1 ->
                            _allData.indexOfFirst { i2 -> i1.id == i2.id } == -1
                        }
    ...
    private fun applySearchFilter() {
        val kw = searchKeyword.value
        if (kw.isNotBlank()) {
            list.data.value = _allData.filter {
                it.title.contains(kw, ignoreCase = true)
            }.toMutableList()
        } else {
            list.data.value = _allData.toMutableList()
        }
    }
