...
internal fun SearchItemCard(
    ...
    when (cardItem) {
        ...
        is CardItem.Article -> {
            val articleItem = cardItem.value
            ArticleItemBox(
                modifier = Modifier.padding(
                    horizontal = 10.dp,
                    vertical = 5.dp
                ...
                cover = articleItem.imageUrls.firstOrNull() ?: articleItem.cover,
                author = articleItem.author.ifBlank { articleItem.name },
                viewNum = NumberUtil.converString(articleItem.view.toString()),
                likeNum = NumberUtil.converString(articleItem.like.toString()),
                replyNum = NumberUtil.converString(articleItem.reply.toString()),
