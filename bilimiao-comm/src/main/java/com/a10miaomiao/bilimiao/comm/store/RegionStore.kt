...
class RegionStore(override val di: DI) :
    ...
    private fun readRegionJson(
        ...
        return try {
            val inputStream = if (!File(context.filesDir, "region.json").exists()) {
            ...
            miaoLogger().e("读取region.json失败", e)
            null
        }
    }
    ...
    private fun writeRegionJson(context: Context, region: ResponseData<List<RegionInfo>>) {
        try {
            ...
            context.openFileOutput("region.json", Context.MODE_PRIVATE).use { outputStream ->
                outputStream.write(jsonStr.toByteArray())
            }
        } catch (e: IOException) {
            miaoLogger().e("写入region.json失败", e)
        }
    }
