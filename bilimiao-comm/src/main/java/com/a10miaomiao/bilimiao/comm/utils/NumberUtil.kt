...
object NumberUtil {
    ...
    /**
     * 反向解析播放量文本为数字（个为单位）
     * "10万" → 100000, "1.2万" → 12000, "2345" → 2345
     */
    fun parseToLong(text: String?): Long {
        if (text.isNullOrBlank()) return 0L
        val cleaned = text.replace("观看", "").replace(",", "").trim()
        return try {
            if (cleaned.endsWith("亿")) {
                (cleaned.dropLast(1).toDouble() * 100_000_000).toLong()
            } else if (cleaned.endsWith("万")) {
                (cleaned.dropLast(1).toDouble() * 10_000).toLong()
            } else {
                cleaned.toLong()
            }
