...
//        backgroundColor = ctx.config.colorSurfaceVariant
    ...
    override fun updateTheme(color: Int, bgColor: Int, fgColor: Int) {
        root.backgroundColor = bgColor
        mTitle.setTextColor(fgColor)
        mNavigationIcon.imageTintList = ColorStateList.valueOf(fgColor)
        mNavigationPointerIcon.imageTintList = ColorStateList.valueOf(fgColor)
        mNavigationExchangeIcon.imageTintList = ColorStateList.valueOf(fgColor)
        ...
        for (i in 0 until mNavigationMemuLayout.childCount) {
            ...
            when (view) {
                is MenuCheckableItemView -> {
                    view.themeColor = color
                    view.fgColor = fgColor
                    view.invalidate()
                }
                is MenuItemView -> {
                    view.refreshTheme(fgColor)
                    view.invalidate()
                }
            }
        }
